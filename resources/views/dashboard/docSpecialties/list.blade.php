        ...
        <div class="row">
            ...
            <div class="box box-primary box-solid">
                    <div class="box-header box-header-background with-border">
                        <h3 class="box-title">تخصص الطبيب</h3>
                        <div class="box-tools pull-right">
                            <a href="{{ url('dashboard/docSpecialties/create ')}}" class="btn btn-warning" style="padding-bottom: 3px;"> <i class="fa fa-plus" aria-hidden="true"></i>&nbsp; أضافه جديد</a>
                        </div>
                    </div>

                    <div class="box-body table-responsive">
                        <table id="docDegrees_table" class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>رقم التسلسل</th>
                                    <th>أسم درجه الطبيب</th>
                                    <th class="">الحدث</th>
                                </tr>
                            </thead>
                        </table>
...
@section('js')
    ...
    $(document).ready(function () {
        ...
        //$(window).bind("load", function() {
            ...
            if ($('#docDegrees_table').length > 0) {
            var tableData = $('#docDegrees_table').DataTable({
                    //stateSave: true,
                    ...
                    dom: 'lBfrtip',
                    ajax: {
                        url: "{{ route('docSpecialties.index') }}",
                        type: 'GET',
                        data: function (d) {
                        }
                    ...
                    "fnDrawCallback": function (oSettings) {
                        ...
                        $('body').off('click', '[id^="changeStatus-"]').on('click', '[id^="changeStatus-"]', function (e) {
                            var self = $(this);
                            var tbl = "<?php echo 'docSpecialties' ?>";
                    ...
                    columns: [
                        {data: 'SPECIALT_ID', name: 'SPECIALT_ID', 'visible': false},
